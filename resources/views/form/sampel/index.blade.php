    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3><i class="bi bi-list-check"></i> Pengambilan Sampel</h3>
                <div>
                    @can('can access add button')
                    <a href="{{ route('sampel.create') }}" class="btn btn-success me-2">
                        <i class="bi bi-plus-circle"></i> Tambah
                    </a>
                    @endcan
                    <a href="{{ route('sampel.exportPdf', ['date' => request('date')]) }}" target="_blank" class="btn btn-primary">
            ...
            <form id="filterForm" method="GET" action="{{ route('sampel.index') }}" class="d-flex flex-wrap align-items-center gap-2 mb-3 p-2 border rounded bg-light shadow-sm">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle">
                                ...
                                <a href="{{ route('sampel.edit.form', $dep->uuid) }}" class="btn btn-warning btn-sm me-1 mb-1">
                                    <i class="bi bi-pencil-square"></i> Edit Data
                                </a>
                                @endcan
                                @can('can access update button')
                                <a href="{{ route('sampel.update.form', $dep->uuid) }}" class="btn btn-info btn-sm me-1 mb-1">
                                    <i class="bi bi-pencil"></i> Update
                                </a>
                                @endcan
                                @can('can access delete button')
                                <form action="{{ route('sampel.destroy', $dep->uuid) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1" aria-labelledby="verifyModalLabel{{ $dep->uuid }}" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-md">
                                    <form action="{{ route('sampel.verification.update', $dep->uuid) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                    ...
                    @empty
                    <tr>
                        <td colspan="19" class="text-center">Belum ada data sampel.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
