...
<div class="container-fluid py-0">
	...
	<div class="d-sm-flex justify-content-between align-items-center mb-4">
		<h2 class="h4">Data Sampling Produk</h2>
		<div class="btn-group" role="group">
			...
			<a href="{{ route('sampling.exportPdf', ['date' => request('date'), 'shift' => request('shift'), 'nama_produk' => request('nama_produk')]) }}" target="_blank" class="btn btn-danger">
				<i class="bi bi-file-earmark-pdf"></i> Export PDF
			</a>
			@endcan
			@can('can access recycle')
			<a href="{{ route('sampling.recyclebin') }}" class="btn btn-secondary">
	...
	<form id="filterForm" method="GET" action="{{ route('sampling.index') }}" class="d-flex flex-wrap align-items-center gap-2 mb-3 p-3 border rounded bg-white shadow-sm">
		<div class="row">
			<div class="col-md-3">
					...
					<tbody>
						...
						<td class="text-center align-middle">
							...
							<td class="text-center align-middle">
								...
								<a href="{{ route('sampling.edit.form', $dep->uuid) }}" class="btn btn-warning btn-sm me-1 mb-1">
									<i class="bi bi-pencil-square"></i> Edit Data
								</a>
								@endcan
								@can('can access update button')
								<a href="{{ route('sampling.update.form', $dep->uuid) }}" class="btn btn-info btn-sm me-1 mb-1">
									<i class="bi bi-pencil"></i> Update
								</a>
								@endcan
								@can('can access delete button')
								<form action="{{ route('sampling.destroy', $dep->uuid) }}" method="POST" class="d-inline">
									@csrf
									@method('DELETE')
							...
							<div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1" aria-labelledby="verifyModalLabel{{ $dep->uuid }}" aria-hidden="true">
								<div class="modal-dialog modal-dialog-centered modal-md">
									<form action="{{ route('sampling.verification.update', $dep->uuid) }}" method="POST">
										@csrf
										@method('PUT')
