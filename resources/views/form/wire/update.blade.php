...
<div class="container-fluid py-4">
    <div class="card shadow-sm">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('wire.update_qc', $wire->uuid) }}">
                @csrf
                @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                value="{{ old('date', $wire->date) }}"
                                {{ $wire->date ? 'readonly' : '' }} required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Shift</label>
                                <select name="shift" class="form-control" {{ $wire->shift ? 'disabled' : '' }} required>
                                ...
                                @if($wire->shift)
                                <input type="hidden" name="shift" value="{{ $wire->shift }}">
                                @endif
                            </div>
                        </div>
                        <div class="row mb-3">
                            ...
                            @if($wire->nama_produk)
                            <input type="hidden" name="nama_produk" value="{{ $wire->nama_produk }}">
                            @endif
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Nama Supplier</label>
                            <select name="nama_supplier" class="form-control selectpicker" data-live-search="true"
                            {{ $wire->nama_supplier ? 'disabled' : '' }} required>
                        ...
                        @if($wire->nama_supplier)
                        <input type="hidden" name="nama_supplier" value="{{ $wire->nama_supplier }}">
                        @endif
                    </div>
                </div>
            </div>
                    ...
                    <tbody id="wireBody">
                        ...
                        <tr class="{{ $bIndex==0 ? 'mesin-row':'batch-row' }}">
                            ...
                            <td rowspan="{{ $rowspan }}" class="mesin-cell">
                                <select name="data_wire[{{ $mIndex }}][mesin]"
                                class="form-control form-control-sm"
                                {{ !empty($mesinData['mesin']) ? 'disabled' : '' }} required>
                            ...
                            @if(!empty($mesinData['mesin']))
                            <input type="hidden" name="data_wire[{{ $mIndex }}][mesin]" value="{{ $mesinData['mesin'] }}">
                            @endif
                        </td>
                        @endif
                        <td>
                            <input type="time" name="data_wire[{{ $mIndex }}][detail][{{ $bIndex }}][start]"
                            class="form-control form-control-sm"
                            value="{{ $batch['start']??'' }}"
                            {{ !empty($batch['start']) ? 'readonly' : '' }}>
                        </td>
                        <td>
                            <input type="time" name="data_wire[{{ $mIndex }}][detail][{{ $bIndex }}][end]"
                            class="form-control form-control-sm"
                            value="{{ $batch['end']??'' }}"
                            {{ !empty($batch['end']) ? 'readonly' : '' }}>
                        </td>
                        <td>
                            <input type="text" name="data_wire[{{ $mIndex }}][detail][{{ $bIndex }}][no_lot]"
                            class="form-control form-control-sm"
                            value="{{ $batch['no_lot']??'' }}"
                            {{ !empty($batch['no_lot']) ? 'readonly' : '' }}>
                        </td>
                        <td>
                            @if($bIndex==0)
                            ...
                            @else
                            <button type="button" class="btn btn-danger btn-sm removeRow"
                            {{ !empty($batch['no_lot']) ? 'disabled' : '' }}>Hapus</button>
                            @endif
                        </td>
                    </tr>
                    @endforeach
