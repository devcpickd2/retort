...
<div class="container-fluid py-0">
    ...
    <div class="d-sm-flex justify-content-between align-items-center mb-4">
        <h2 class="h4">Kontrol Labelisasi Karton</h2>
        <div class="btn-group" role="group">
            ...
            <a href="{{ route('karton.recyclebin') }}" class="btn btn-secondary">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle">{{\Carbon\Carbon::parse($dep->waktu_mulai)->format('H:i') }} - {{\Carbon\Carbon::parse($dep->waktu_selesai)->format('H:i') }}</td>
                            ...
                            <a href="{{ route('karton.edit.form', $dep->uuid) }}"
                                class="btn btn-warning btn-sm me-1 mb-1">
                                <i class="bi bi-pencil-square"></i> Edit Data
                            </a>
                            @endcan
                            @can('can access update button')
                            <a href="{{ route('karton.update.form', $dep->uuid) }}"
                                class="btn btn-info btn-sm me-1 mb-1">
                                <i class="bi bi-pencil"></i> Update
                            </a>
                            @endcan
                            @can('can access delete button')
                            <form action="{{ route('karton.destroy', $dep->uuid) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                        ...
                        <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1"
                            ...
                            <div class="modal-dialog modal-dialog-centered modal-md">
                                <form action="{{ route('karton.verification.update', $dep->uuid) }}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
