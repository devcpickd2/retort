...
@section('title', 'Verifikasi Penyimpangan (Diketahui)')
        ...
        <div class="card-body p-4">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h4 class="card-title mb-0 fw-bold">
                    <i class="fas fa-check-double me-2"></i>Verifikasi Penyimpangan (Diketahui Oleh)
                </h4>
            </div>
            ...
            {{-- BAGIAN FILTER --}}
            <form method="GET" action="{{ route('penyimpangan-kualitas.verification.diketahui') }}" class="row g-3 align-items-end mb-4">
                <div class="col-md-3">
                ...
                <div class="col-md-3 d-flex gap-2">
                    ...
                    <a href="{{ route('penyimpangan-kualitas.verification.diketahui') }}" class="btn btn-secondary w-100"><i class="fas fa-sync-alt me-1"></i> Reset</a>
                </div>
            </form>
        </div>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <button type="button" class="btn btn-primary btn-sm fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#verifyModal{{ $item->id }}">
                                    ...
                                    {{ $item->status_diketahui != 0 ? 'Re-Verifikasi' : 'Verifikasi' }}
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-5">
                                ...
                                <p class="small text-muted">Tidak ada data yang menunggu verifikasi Anda.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<div class="modal fade modal-verification" id="verifyModal{{ $item->id }}" tabindex="-1" aria-labelledby="verifyModalLabel{{ $item->id }}" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="{{ route('penyimpangan-kualitas.verify.diketahui', $item->id) }}" method="POST">
                @csrf
                @method('POST')
                <div class="modal-header">
                    <h5 class="modal-title fw-bolder fs-4 text-uppercase" id="verifyModalLabel{{ $item->id }}"><i class="bi bi-gear-fill me-2"></i>Verification (Diketahui)</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <p class="text-light mb-4">Verifikasi data untuk: <strong>{{ $item->nomor }}</strong> (Produk: {{ $item->nama_produk }})</p>
                    <div class="mb-4">
                        <label for="status_diketahui_{{ $item->id }}" class="form-label fw-bold d-block text-center mb-2">Pilih Status Verifikasi</label>
                        <select name="status_diketahui" id="status_diketahui_{{ $item->id }}" class="form-select form-select-lg fw-bold text-center mx-auto form-select-custom" style="width: 85%;" required>
                            <option value="1" {{ old('status_diketahui', $item->status_diketahui) != 2 ? 'selected' : '' }} style="color: #198754;">✅ Verified (Diketahui)</option>
                            <option value="2" {{ old('status_diketahui', $item->status_diketahui) == 2 ? 'selected' : '' }} style="color: #dc3545;">❌ Revision (Perlu Perbaikan)</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="catatan_diketahui_{{ $item->id }}" class="form-label fw-bold">Catatan Verifikasi (Wajib jika 'Revision')</label>
                        <textarea name="catatan_diketahui" id="catatan_diketahui_{{ $item->id }}" rows="4"
                            class="form-control shadow-none textarea-custom"
                            placeholder="Tuliskan catatan perbaikan...">{{ old('catatan_diketahui', $item->catatan_diketahui) }}</textarea>
                    </div>
                </div>
